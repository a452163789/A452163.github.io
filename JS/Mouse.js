function init() {
    ...
    class ArrowPointer {
        constructor() {
            ...
            this.degrees = 57.296;
            this.cursorSize = 20;
            this.dragFactor = 0.0; // 调整这个值来改变拖拽感
            this.easingFactor = 0.1; // 调整这个值来改变缓动效果
        ...
        move(event) {
            this.position.targetX = event.pageX;
            this.position.targetY = event.pageY;
        }

        animate() {
            this.position.distanceX = this.position.targetX - this.position.pointerX;
            this.position.distanceY = this.position.targetY - this.position.pointerY;
            this.position.pointerX += this.position.distanceX * this.easingFactor;
            this.position.pointerY += this.position.distanceY * this.easingFactor;

            this.cursor.style.transform = `translate3d(${this.position.pointerX}px, ${this.position.pointerY}px, 0)`;

            if (Math.abs(this.position.distanceX) > 1 || Math.abs(this.position.distanceY) > 1) {
